...
function XpraWindow(client, canvas_state, wid, x, y, w, h, metadata,
  ...
  if (this.client.server_is_desktop || this.client.server_is_shadow) {
  ...
  } else if ((this.windowtype == "") || (this.windowtype == "NORMAL") ||
             (this.windowtype == "DIALOG") || (this.windowtype == "UTILITY")) {
    this.resizable = true;
    ...
    jQuery(this.div).on("resizestop", function(ev, ui) {
      ...
      setTimeout(function() { me.client.request_refresh(me.wid); }, 200);
    });
    ...
    if (this.resizable) {
      jQuery(this.d_header).dblclick(function() { me.toggle_maximized(); });
      jQuery(this.d_closebtn).click(function() { window_closed_cb(me); });
      jQuery(this.d_maximizebtn).click(function() { me.toggle_maximized(); });
      jQuery(this.d_minimizebtn).click(function() { me.toggle_minimized(); });
    } else {
    ...
    jQuery(this.div).mousedown(function(e) { e.stopPropagation(); });
  ...
  if (window.PointerEvent) {
    ...
    this.canvas.addEventListener("pointermove", function(ev) {
      ...
      if (ev.pointerType == "touch") {
        if (me.is_scrolling == 1) {
          ...
          var dy = 0;
          if (ev.isPrimary) {
            dx = Math.abs(ev.screenX - me.nonprimary_pointer.screenX) -
                 Math.abs(me.primary_pointer.screenX -
                          me.nonprimary_pointer.screenX);
            dy = Math.abs(me.primary_pointer.screenY -
                          me.nonprimary_pointer.screenY) -
                 Math.abs(ev.screenY - me.nonprimary_pointer.screenY);
            me.primary_pointer = ev;
          } else {
            dx = Math.abs(me.primary_pointer.screenX - ev.screenX) -
                 Math.abs(me.primary_pointer.screenX -
                          me.nonprimary_pointer.screenX);
            dy = Math.abs(me.primary_pointer.screenY -
                          me.nonprimary_pointer.screenY) -
                 Math.abs(me.primary_pointer.screenY - ev.screenY);
            me.nonprimary_pointer = ev;
          }
    ...
    jQuery(this.canvas).mousedown(function(e) { me.on_mousedown(e); });
    jQuery(this.canvas).mouseup(function(e) { me.on_mouseup(e); });
    jQuery(this.canvas).mousemove(function(e) { me.on_mousemove(e); });
  }
...
XpraWindow.prototype.ensure_visible = function() {
  ...
  // this.log("x=", this.x, "y=", this.y, "w=", this.w, "h=", this.h,
  // "leftoffset=", this.leftoffset, "topoffset=", this.topoffset, " - ww=", ww,
  // "wh=", wh);
  ...
  this.debug("geometry", "ensure_visible() oldx=", oldx, "oldy=", oldy,
             "x=", this.x, "y=", this.y);
...
XpraWindow.prototype.updateCSSGeometry = function() {
  ...
  this.debug("geometry", "updateCSSGeometry() left=", this.outerX,
             ", top=", this.outerY, ", width=", this.outerW,
             ", height=", this.outerH);
};
...
XpraWindow.prototype.updateFocus = function() {
  if (this.focused) {
    ...
    // Update window title
    // jQuery("title").text(location.pathname.replaceAll("/","") + ": " +
    // this.title);

    // Update the icon
    if (this.icon !== null) {
      const src = this.update_icon(this.icon.width, this.icon.height,
                                   this.icon.encoding, this.icon.img_data);
 ...
 */
XpraWindow.prototype.toString =
    function() { return "Window(" + this.wid + ")"; };
  ...
  if ("title" in metadata) {
    ...
    if (this.client.packet_encoder != "rencodeplus") {
      try {
        ...
        this.log("unable to decode title string '" + title + "' received from ",
                 this.client.protocol.packet_encoder, ": " + e);
      }
    }
    if (this.title != title) {
      this.title = title;
  ...
  if ("class-instance" in metadata) {
    ...
    if (classes) {
      ...
      for (let i = 0; i < classes.length; i++) {
        ...
        if (tclass.indexOf("wmclass-") === 0 && wm_class &&
...
XpraWindow.prototype.save_geometry = function() {
  ...
  this.debug("geometry",
             "save_geometry() saved-geometry=", this.saved_geometry);
};
/**
...
XpraWindow.prototype.restore_geometry = function() {
  ...
  this.debug("geometry",
             "restore_geometry() saved-geometry=", this.saved_geometry);
 ...
 */
XpraWindow.prototype.toggle_maximized =
    function() { this.set_maximized(!this.maximized); };
...
XpraWindow.prototype.toggle_minimized = function() {
  ...
  if (this.minimized) {
    ...
    this.client.send([
      "map-window", this.wid, geom.x, geom.y, geom.w, geom.h,
...
XpraWindow.prototype._set_decorated = function(decorated) {
  ...
  if (decorated) {
    ...
    if (this.d_header) {
      this.topoffset =
          this.topoffset + parseInt(jQuery(this.d_header).css('height'), 10);
      this.debug("geometry", "_set_decorated(", decorated,
                 ") new topoffset=", self.topoffset);
    }
  } else {
...
XpraWindow.prototype.handle_moved = function(e) {
  const left = Math.round(e.position.left), top = Math.round(e.position.top);
...
XpraWindow.prototype.screen_resized = function() {
  this.debug("geometry", "screen_resized() server_is_desktop=",
...
XpraWindow.prototype.recenter = function(force_update_geometry) {
  ...
  this.debug("geometry", "recenter() x=", x, ", y=", y,
             ", desktop size: ", this.client.desktop_width,
...
XpraWindow.prototype.initiate_moveresize = function(
    mousedown_event, x_root, y_root, direction, button, source_indication) {
  ...
  if (direction == MOVERESIZE_MOVE && mousedown_event) {
    ...
    if (resize_widget) {
      ...
      resize_widget.trigger(
          {type : "mousedown", which : 1, pageX : pageX, pageY : pageY});
    }
  }
};
 ...
 */
XpraWindow.prototype.handle_mouse_click = function(button, pressed, mx, my,
                                                   modifiers, buttons) {
...
XpraWindow.prototype.update_icon = function(width, height, encoding, img_data) {
  ...
  if (encoding == "png") {
    ...
    src = "data:image/" + encoding + ";base64," +
...
XpraWindow.prototype.set_cursor = function(encoding, w, h, xhot, yhot,
                                           img_data) {
  if (encoding != "png") {
  ...
  // j.src = "data:image/"+coding+";base64," +
  // Utilities.ArrayBufferToBase64(img_data);
 ...
 * canvas. However the drawing is requested by requestAnimationFrame which
 * allows the browser to group screen redraws together, and automatically
 * adjusts the framerate e.g if the browser window/tab is not visible.
 */
XpraWindow.prototype.draw = function() {
  // pass the 'buffer' canvas directly to visible canvas context
  if (this.has_alpha || this.tray) {
    this.canvas_ctx.clearRect(0, 0, this.draw_canvas.width,
 ...
 */
XpraWindow.prototype._init_broadway = function(enc_width, enc_height, width,
                                               height) {
  const me = this;
  ...
  this.broadway_decoder.onPictureDecoded = function(buffer, p_width, p_height,
                                                    infos) {
    me._broadway_paint(buffer, enc_width, enc_height, width, height, p_width,
                       p_height, infos);
  };
};

XpraWindow.prototype._broadway_paint = function(
    buffer, enc_width, enc_height, width, height, p_width, p_height, infos) {
  this.debug("draw", "broadway picture decoded: ", buffer.length,
             "bytes, size ", p_width, "x",
             p_height + ", paint location: ", this.broadway_paint_location,
             "with infos=", infos);
  ...
  if (enc_width != width || enc_height != height) {
    // scale it:
    this.offscreen_canvas_ctx.drawImage(this.offscreen_canvas, x, y, enc_width,
                                        enc_height, x, y, width, height);
  }
};

XpraWindow.prototype._close_broadway =
    function() { this.broadway_decoder = null; };

XpraWindow.prototype._close_video = function() {
  this.debug("draw",
             "close_video: video_source_buffer=", this.video_source_buffer,
             ", media_source=", this.media_source, ", video=", this.video);
...
XpraWindow.prototype._init_video = function(width, height, coding, profile,
                                            level) {
  const me = this;
  ...
  if (coding == "h264+mp4" || coding == "mpeg4+mp4") {
    ...
    codec_string = 'video/mp4; codecs="avc1.' +
                   MediaSourceConstants.H264_PROFILE_CODE[profile] +
                   MediaSourceConstants.H264_LEVEL_CODE[level] + '"';
  ...
  this.log("video codec string: " + codec_string + " for " + coding +
  ...
  this.media_source.addEventListener('sourceopen', function() {
    ...
    vsb.addEventListener(
        'error', function(e) { me.error("video source buffer error"); });
    vsb.addEventListener('waiting', function() { me._push_video_buffers(); });
    // push any buffers that may have accumulated since we initialized the video
    // element:
...
XpraWindow.prototype._non_video_paint = function(coding) {
  if (this.video && this.video.style.zIndex != "-1") {
    this.debug("draw", "bringing canvas above video for ", coding,
               " paint event");
...
XpraWindow.prototype.may_paint_now = function paint() {
  ...
  while ((this.paint_pending == 0 || (now - this.paint_pending) >= 2000) &&
...
XpraWindow.prototype.do_paint = function paint(
    x, y, width, height, coding, img_data, packet_sequence, rowstride, options,
    decode_callback) {
  const me = this;
  ...
  if (bitmap) {
    ...
    this.debug("draw", coding, img_data, " at ", "" + x + "," + y,
               ") focused=", this.focused);
  } else {
    this.debug("draw", "do_paint(", img_data.length, " bytes of", coding,
               " data ", width, "x", height, " at ", x, ",", y,
               ") focused=", this.focused);
  }
  if (scaled_size) {
  ...
  function paint_bitmap() {
    ...
    // when decoding in the worker (bitmaps can only come from the decode
    // worker)
    me.may_paint_now();
  }
  ...
  try {
    if (coding == "void") {
      ...
      this.debug("draw", "got ", img_data.length, "bytes of", coding,
                 "to paint with stride", rowstride, ", target stride",
                 target_stride);
      if (coding == "rgb24") {
        const uint = new Uint8Array(target_stride * enc_height);
        let i = 0, j = 0, l = img_data.length;
      ...
      if (rowstride > target_stride) {
        img = this.offscreen_canvas_ctx.createImageData(
            Math.round(rowstride / 4), enc_height);
      } else {
      ...
      if (enc_width != width || enc_height != height) {
        // scale it:
        this.offscreen_canvas_ctx.drawImage(this.offscreen_canvas, x, y,
                                            enc_width, enc_height, x, y, width,
                                            height);
      }
      painted();
      this.may_paint_now();
      ...
      this.debug("draw", coding, "frame", frame, "data len=", img_data.length,
                 "decoded=", decoded);
      ...
      painted();
    } else if (coding == "h264+mp4" || coding == "vp8+webm" ||
               coding == "mpeg4+mp4") {
      ...
      if (img_data.length > 0) {
        this.debug("draw", "video state=",
                   MediaSourceConstants.READY_STATE[this.video.readyState],
                   ", network state=",
                   MediaSourceConstants.NETWORK_STATE[this.video.networkState]);
        this.debug("draw", "video paused=", this.video.paused,
                   ", video buffers=", this.video_buffers.length);
      ...
      for (let i = 0, j = img_data.length; i < j; ++i) {
        ...
        const sx = scroll_data[0], sy = scroll_data[1], sw = scroll_data[2],
              sh = scroll_data[3], xdelta = scroll_data[4],
              ydelta = scroll_data[5];
        this.offscreen_canvas_ctx.drawImage(this.draw_canvas, sx, sy, sw, sh,
