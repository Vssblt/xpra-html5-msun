...
$(function() {
  ...
  window.doNotification = function(type, nid, title, message, timeout, icon,
                                   actions, hints, onAction, onClose) {
    console.debug("doNotification", type, nid, title, message, timeout, icon,
                  actions, hints, onAction, onClose);
    ...
    if (actions) {
      ...
      for (let i = 0; i < actions.length; i += 2) {
        ...
        const notification_button = window._notification_button(
            nid, action_id, action_label, onAction, onClose);
    ...
    if (icon) {
      ...
      if (encoding == "png") {
        const src =
            "data:image/" + encoding + ";base64," + window.btoa(img_data);
    ...
    setTimeout(function() { a.addClass('visible'); }, 1);
  ...
  window._notification_button = function(nid, action_id, action_label, onAction,
                                         onClose) {
    const notification_button =
        $('<div class="notification_button" id=notification"' + action_id +
          '">' + action_label + '</div>');
