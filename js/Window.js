...
function XpraWindow(client, canvas_state, wid, x, y, w, h, metadata,
  ...
  if (this.client.server_is_desktop || this.client.server_is_shadow) {
    ...
    jQuery(this.d_header).click(function() { me.set_focus_cb(me); });
  }
 ...
 */
XpraWindow.prototype.set_minimized = function(
    minimized) { jQuery(this.div).toggle(200); };

/**
 * Toggle minimized state
 */
XpraWindow.prototype.toggle_minimized =
    function() { this.set_minimized(!this.minimized); };
...
XpraWindow.prototype.set_cursor = function(encoding, w, h, xhot, yhot,
                                           img_data) {
  if (encoding == "png") {
    var cursor_url = "url('data:image/" + encoding + ";base64," +
                     window.btoa(img_data) + "')";
    var window_element = jQuery("#" + String(this.wid));
    window_element.css("cursor", cursor_url + ", default");
    // CSS3 with hotspot:
    window_element.css("cursor",
                       cursor_url + " " + xhot + " " + yhot + ", auto");
...
XpraWindow.prototype.do_paint = function paint(
    ...
    decode_callback) {
  this.debug("draw", "do_paint(", img_data.length, " bytes of ",
             ("zlib" in options ? "zlib " : ""), coding, " data ", width, "x",
             height, " at ", x, ",", y, ") focused=", this.focused);
  var me = this;
  ...
  try {
    if (coding == "void") {
      ...
      if (coding == "rgb24") {
        ...
        let i = 0, j = 0, k = 0, l = img_data.length;
      ...
      for (var i = 0, j = img_data.length; i < j; ++i) {
        ...
        var sx = scroll_data[0], sy = scroll_data[1], sw = scroll_data[2],
