...
function XpraClient(container) {
  ...
  if (window.jQuery) {
    ...
    jQuery(window).resize(
        jQuery.debounce(250, function(e) { me._screen_resized(e, me); }));
  }
...
XpraClient.prototype.init_settings = function(container) {
  ...
  this.supported_encodings = [
    "jpeg", "png", "rgb", "rgb32", "rgb24"
  ]; //"h264", "vp8+webm", "h264+mp4", "mpeg4+mp4"];
...
XpraClient.prototype.init_state = function(container) {
  ...
  this.audio_aurora_enabled = typeof AV !== 'undefined' && AV != null &&
                              AV.Decoder != null &&
  ...
  if (CLIPBOARD_IMAGES && navigator.clipboard &&
      navigator.clipboard.hasOwnProperty("write")) {
    this.clipboard_targets.push("image/png");
  } else {
    this.log("no clipboard write support: no images, navigator.clipboard=",
             navigator.clipboard);
  }
  ...
  screen_element.mousedown(function(e) { me.on_mousedown(e); });
  screen_element.mouseup(function(e) { me.on_mouseup(e); });
  screen_element.mousemove(function(e) { me.on_mousemove(e); });

  const div = document.getElementById("screen");
  function on_mousescroll(e) { me.on_mousescroll(e); }
  if (Utilities.isEventSupported("wheel")) {
    ...
    div.addEventListener('DOMMouseScroll', on_mousescroll,
                         false); // for Firefox
  }
};
...
XpraClient.prototype.cerror =
    function() { Utilities.cerror.apply(Utilities, arguments); };
XpraClient.prototype.warn = function() {
  // logging.WARN = 30
  this.send_log(30, arguments);
  this.cwarn.apply(this, arguments);
};
XpraClient.prototype.cwarn =
    function() { Utilities.cwarn.apply(Utilities, arguments); };
XpraClient.prototype.log = function() {
  // logging.INFO = 20
  this.send_log(20, arguments);
  this.clog.apply(this, arguments);
};
XpraClient.prototype.clog =
    function() { Utilities.clog.apply(Utilities, arguments); };
...
XpraClient.prototype.cdebug =
    function() { Utilities.cdebug.apply(Utilities, arguments); };
...
XpraClient.prototype.on_connection_progress = function(state, details,
                                                       progress) {
  // can be overriden
...
XpraClient.prototype._screen_resized = function(event, ctx) {
  ...
  const packet =
      [ "desktop_size", newsize[0], newsize[1], this._get_screen_sizes() ];
  ctx.send(packet);
...
XpraClient.prototype.translate_modifiers = function(modifiers) {
  ...
  // this.clog("altgr_state=", this.altgr_state, ", altgr_modifier=",
  // this.altgr_modifier, ", modifiers=", new_modifiers);
...
XpraClient.prototype._check_browser_language = function(key_layout) {
  ...
  if (key_layout) {
    ...
    if (l && this.browser_language != l) {
      ...
      this.clog("browser language changed from", this.browser_language, "to",
                l);
  ...
  if (new_layout != null && this.key_layout != new_layout) {
    ...
    this.clog("keyboard layout changed from", this.key_layout, "to",
              key_layout);
...
XpraClient.prototype._keyb_process =
  ...
  this.debug("keyboard", "last keycode pressed=", this.last_keycode_pressed,
             ", keycode=", keycode, ", pressed=", pressed, ", str=", str);
  ...
  if (pressed && Utilities.isMacOS() && raw_modifiers.includes("meta") &&
      ostr != "meta") {
    unpress_now = true;
  }
  ...
  if (this.clipboard_enabled) {
    ...
    if (clipboard_modifier_keys.indexOf(keyname) >= 0) {
      this.debug("keyboard",
                 "passing clipboard modifier key event to browser:", keyname);
      allow_default = true;
    }
    ...
    if (clipboard_mod_set) {
      ...
      if (l == "c" || l == "x" || l == "v") {
        this.debug("keyboard", "passing clipboard combination to browser:",
                   clipboard_modifier, "+", keyname);
        allow_default = true;
        if (l == "v") {
          this.clipboard_delayed_event_time =
              Utilities.monotonicTime() + CLIPBOARD_EVENT_DELAY;
        }
      }
    }
  }
  ...
  if (this.topwindow != null) {
    let packet = [
      "key-action", this.topwindow, keyname, pressed, modifiers, keyval, str,
      keycode, group
    ];
    this.key_packets.push(packet);
    if (unpress_now) {
      packet = [
        "key-action", this.topwindow, keyname, false, modifiers, keyval, str,
        keycode, group
      ];
      this.key_packets.push(packet);
    }
...
XpraClient.prototype._keyb_onkeydown = function(
    event, ctx) { return ctx._keyb_process(true, event); };
XpraClient.prototype._keyb_onkeyup = function(
    event, ctx) { return ctx._keyb_process(false, event); };

XpraClient.prototype._get_keyboard_layout = function() {
  this.debug("keyboard",
             "_get_keyboard_layout() keyboard_layout=", this.keyboard_layout);
...
XpraClient.prototype._get_screen_sizes = function() {
  ...
  const screen = [
    "HTML", screen_size[0], screen_size[1], wmm, hmm, [ monitor ], 0, 0,
...
XpraClient.prototype._check_server_echo = function(ping_sent_time) {
  ...
  // this.clog("check_server_echo", this.server_ok, "last", last, "last_time",
  // this.last_ping_echoed_time, "this_this", ping_sent_time);
...
XpraClient.prototype._check_echo_timeout = function(ping_time) {
  ...
  if (this.last_ping_echoed_time < ping_time) {
    if (this.reconnect && this.reconnect_attempt < this.reconnect_count) {
      ...
      this.callback_close("server ping timeout, waited " + this.PING_TIMEOUT +
...
XpraClient.prototype.emit_connection_lost = function(
    event_type) { this._emit_event("connection-lost"); };
XpraClient.prototype.emit_connection_established = function(
    event_type) { this._emit_event("connection-established"); };
...
XpraClient.prototype._send_hello = function(challenge_response, client_salt) {
  ...
  for (const key in this.capabilities) {
    ...
    if (key == null) {
      ...
      throw new Error("invalid null value for key " + key +
                      " in hello packet data");
    }
  }
  // send the packet
...
XpraClient.prototype._make_hello = function() {
  ...
  this._update_capabilities({
    ...
    "metadata.supported" : [
      ...
      "has-alpha", "decorations", "override-redirect", "tray", "modal",
      "opacity",
      //"shadow", "desktop",
    ],
...
XpraClient.prototype.on_mousemove = function(
    e) { this.do_window_mouse_move(e, null); };

XpraClient.prototype.on_mousedown = function(
    e) { this.do_window_mouse_click(e, null, true); };

XpraClient.prototype.on_mouseup = function(
    e) { this.do_window_mouse_click(e, null, false); };

XpraClient.prototype.on_mousescroll = function(
    e) { this.do_window_mouse_scroll(e, null); };
...
XpraClient.prototype.do_window_mouse_scroll = function(e, window) {
  ...
  if (this.server_precise_wheel) {
    if (apx > 0) {
      ...
      this.send(
          [ "wheel-motion", wid, btn_x, -xdist, [ x, y ], modifiers, buttons ]);
    }
    if (apy > 0) {
      const btn_y = (py >= 0) ? 5 : 4;
      const ydist = Math.round(py * 1000 / 120);
      this.send(
          [ "wheel-motion", wid, btn_y, -ydist, [ x, y ], modifiers, buttons ]);
    }
    return;
  }
  ...
  while (wx >= 120) {
    ...
    this.send(
        [ "button-action", wid, btn_x, false, [ x, y ], modifiers, buttons ]);
  }
  while (wy >= 120) {
    wy -= 120;
    this.send(
    ...
    this.send(
        [ "button-action", wid, btn_y, false, [ x, y ], modifiers, buttons ]);
  }
  // store left overs:
...
XpraClient.prototype._poll_clipboard = function(e) {
  ...
  if (navigator.clipboard && navigator.clipboard.readText) {
    ...
    navigator.clipboard.readText().then(
        ...
        function(
            err) { client.debug("clipboard", "paste event failed:", err); });
    return false;
  }
  // fallback code:
...
XpraClient.prototype.packet_disconnect_reason =
    function(packet) {
...
XpraClient.prototype._process_close = function(packet, ctx) {
  ctx.clog("websocket closed: ", packet[1], "reason: ", ctx.disconnect_reason,
           ", reconnect: ", ctx.reconnect,
           ", reconnect attempt: ", ctx.reconnect_attempt);
  if (ctx.reconnect_in_progress) {
    return;
  }
  ctx.packet_disconnect_reason(packet);
...
XpraClient.prototype._connection_change = function(e) {
  const ci = Utilities.getConnectionInfo();
  this.clog("connection status - change event=", e, ", connection info=", ci,
            "tell server:", this.server_connection_data);
...
XpraClient.prototype._process_hello = function(packet, ctx) {
  ...
  if (ctx.audio_enabled) {
    if (!(hello["sound.send"])) {
      ...
      if (!ctx.server_audio_codecs) {
        ...
        ctx.log("audio codecs supported by the server:",
                ctx.server_audio_codecs);
        if (!ctx.server_audio_codecs.includes(ctx.audio_codec)) {
          ctx.warn("audio codec " + ctx.audio_codec +
                   " is not supported by the server");
          ctx.audio_codec = null;
          // find the best one we can use:
          for (let i = 0; i < MediaSourceConstants.PREFERRED_CODEC_ORDER.length;
               i++) {
            const codec = MediaSourceConstants.PREFERRED_CODEC_ORDER[i];
            if ((codec in ctx.audio_codecs) &&
                (ctx.server_audio_codecs.indexOf(codec) >= 0)) {
  ...
  if (navigator.hasOwnProperty("connection")) {
    navigator.connection.onchange = function() { ctx._connection_change(); };
...
XpraClient.prototype.process_xdg_menu = function() {
  ...
  for (key in this.xdg_menu) {
    ...
    a.onmouseenter =
        function() { this.parentElement.childNodes[1].className = "-visible"; };
    a.onmouseleave =
        function() { this.parentElement.childNodes[1].className = ""; };
    ...
    for (key in xdg_menu_cats) {
      ...
      a2.onmouseleave =
          function() { this.parentElement.parentElement.className = ""; };
...
XpraClient.prototype._process_setting_change = function(packet, ctx) {
  const setting = packet[1], value = packet[2];
  if (setting == "xdg-menu") {
    ctx.xdg_menu = value;
    if (ctx.xdg_menu) {
      ctx.process_xdg_menu();
...
XpraClient.prototype.xdg_image = function(icon_data, icon_type) {
  ...
  if (typeof icon_data !== 'undefined') {
    ...
    if (icon_type == "svg") {
      ...
      img.src = "data:image/" + icon_type + ";base64," +
                Utilities.ArrayBufferToBase64(icon_data);
    }
  }
...
XpraClient.prototype._process_challenge = function(packet, ctx) {
  ...
  if (salt_digest == "xor") {
    ...
    if (digest == "xor") {
      if ((!ctx.ssl) && (!ctx.encryption) && (!ctx.insecure) &&
          (ctx.host != "localhost") && (ctx.host != "127.0.0.1")) {
        ctx.callback_close(
  ...
  if (!salt) {
    this.callback_close("server requested an unsupported salt digest " +
                        salt_digest);
    return;
  }
...
XpraClient.prototype._send_ping = function() {
  ...
  this.ping_timeout_timer =
      setTimeout(function() { me._check_echo_timeout(now_ms); },
                 this.PING_TIMEOUT);
  // add timeout to detect temporary ping miss for spinners
  const wait = 2000;
  this.ping_grace_timer =
      setTimeout(function() { me._check_server_echo(now_ms); }, wait);
};
...
XpraClient.prototype._process_ping_echo = function(packet, ctx) {
  ...
  const l1 = packet[2], l2 = packet[3], l3 = packet[4];
  ctx.client_ping_latency = packet[5];
  ctx.server_ping_latency =
      Math.ceil(Utilities.monotonicTime()) - ctx.last_ping_echoed_time;
...
XpraClient.prototype._process_new_tray = function(packet, ctx) {
  ...
  mycanvas.width = w;
  mycanvas.height = h;
  ctx.id_to_window[wid] = new XpraWindow(
...
XpraClient.prototype.send_tray_configure = function(wid) {
  ...
  const w = float_menu_item_size, h = float_menu_item_size;
  this.clog("tray", wid, "position:", x, y);
  this.send([ "configure-window", Number(wid), x, y, w, h, {} ]);
};
XpraClient.prototype._tray_geometry_changed = function(
    win) { win.client.debug("tray", "tray geometry changed (ignored)"); };
XpraClient.prototype._tray_set_focus = function(
    win) { win.client.debug("tray", "tray set focus (ignored)"); };
XpraClient.prototype._tray_closed = function(
    win) { win.client.debug("tray", "tray closed (ignored)"); };
 ...
 * Windows
 */
XpraClient.prototype._new_window = function(
    wid, x, y, w, h, metadata, override_redirect, client_properties) {
  ...
  if (!override_redirect) {
    ...
    this.send([
      "map-window", wid, geom.x, geom.y, geom.w, geom.h, win.client_properties
    ]);
...
XpraClient.prototype._new_window_common = function(packet, override_redirect) {
  ...
  this._new_window(wid, x, y, w, h, metadata, override_redirect,
                   client_properties);
  this._new_ui_event();
};
...
XpraClient.prototype.send_configure_window = function(win, state,
                                                      skip_geometry) {
  const geom = win.get_internal_geometry();
  const wid = win.wid;
  let packet = [
    "configure-window", wid, geom.x, geom.y, geom.w, geom.h,
    win.client_properties, 0, state, skip_geometry
  ];
  this.send(packet);
};

XpraClient.prototype._process_new_window = function(
    packet, ctx) { ctx._new_window_common(packet, false); };

XpraClient.prototype._process_new_override_redirect = function(
    packet, ctx) { ctx._new_window_common(packet, true); };

XpraClient.prototype._process_window_metadata = function(packet, ctx) {
  const wid = packet[1], metadata = packet[2], win = ctx.id_to_window[wid];
  if (win != null) {
...
XpraClient.prototype._process_initiate_moveresize = function(packet, ctx) {
  const wid = packet[1], win = ctx.id_to_window[wid];
  if (win != null) {
    const x_root = packet[2], y_root = packet[3], direction = packet[4],
          button = packet[5], source_indication = packet[6];
    win.initiate_moveresize(ctx.mousedown_event, x_root, y_root, direction,
                            button, source_indication);
  }
};

XpraClient.prototype._process_pointer_position = function(packet, ctx) {
  const wid = packet[1];
  let x = packet[2], y = packet[3];
...
XpraClient.prototype._process_notify_show = function(packet, ctx) {
  ...
  function notify() {
    ...
    const notification =
        new Notification(summary, {body : body, icon : icon_url});
    notification.onclose = function() {
      const reason = 2; // closed by the user - best guess...
      ctx.send([ "notification-close", nid, reason, "" ]);
    };
    notification.onclick =
        function() { ctx.log("user clicked on notification", nid); };
  }
  ...
  if (window.doNotification) {
    window.doNotification(
        "info", nid, summary, body, expire_timeout, icon, actions, hints,
...
XpraClient.prototype._process_draw = function(packet, ctx) {
  if (ctx.queue_draw_packets) {
    if (ctx.dQ_interval_id === null) {
      ctx.dQ_interval_id =
          setInterval(function() { ctx._process_draw_queue(null, ctx); },
...
XpraClient.prototype._process_eos = function(
    packet, ctx) { ctx._process_draw(packet, ctx); };
...
XpraClient.prototype.request_redraw = function(win) {
  ...
  // request that drawing to screen takes place at next available opportunity if
  // possible
  ...
  if (window.requestAnimationFrame) {
    ...
    if (this.draw_pending == 0) {
      ...
      window.requestAnimationFrame(function() {
        me.debug("draw", "animation frame:", me.pending_redraw.length,
                 "windows to paint, processing delay",
                 Utilities.monotonicTime() - me.draw_pending, "ms");
        me.draw_pending = 0;
...
XpraClient.prototype._process_draw_queue = function(packet, ctx) {
  ...
  const ptype = packet[0], wid = packet[1];
  const win = ctx.id_to_window[wid];
  if (ptype == "eos") {
  ...
  const start = Utilities.monotonicTime(), x = packet[2], y = packet[3],
        width = packet[4], height = packet[5], coding = packet[6],
        data = packet[7], packet_sequence = packet[8], rowstride = packet[9];
  let options = {};
  if (packet.length > 10)
    options = packet[10];
  ...
  function send_damage_sequence(decode_time, message) {
    protocol.send([
      "damage-sequence", packet_sequence, wid, width, height, decode_time,
      message
    ]);
  }
  if (!win) {
  ...
  try {
    win.paint(x, y, width, height, coding, data, packet_sequence, rowstride,
              options, function(error) {
                ...
                ctx.debug("draw", "decode time for ", coding, " sequence ",
                          packet_sequence, ": ", decode_time,
                          ", flush=", flush);
...
XpraClient.prototype.init_audio = function(ignore_audio_blacklist) {
  this.debug("audio", "init_audio() enabled=", this.audio_enabled,
             ", mediasource enabled=", this.audio_mediasource_enabled,
             ", aurora enabled=", this.audio_aurora_enabled,
             ", http-stream enabled=", this.audio_httpstream_enabled);
  if (this.audio_mediasource_enabled) {
    this.mediasource_codecs =
        MediaSourceUtil.getMediaSourceAudioCodecs(ignore_audio_blacklist);
  ...
  if (!(this.audio_codec in this.audio_codecs)) {
    ...
    if (this.audio_codec) {
      ...
      if (this.audio_framework) {
        this.log("using " + this.audio_framework +
                 " audio codec: " + this.audio_codec);
      } else {
...
XpraClient.prototype._sound_start_receiving = function() {
  if (!this.audio_framework || !this.audio_codec) {
    ...
    const codecs_supported = MediaSourceUtil.get_supported_codecs(
        this.audio_mediasource_enabled, this.audio_aurora_enabled,
        this.audio_httpstream_enabled, false);
...
XpraClient.prototype._sound_start_mediasource = function() {
  ...
  if (this.debug) {
    MediaSourceUtil.addMediaSourceEventDebugListeners(this.media_source,
                                                      "audio");
  }
  this.media_source.addEventListener(
      'error', function(e) { audio_error("audio source"); });
  ...
  this.media_source.addEventListener('sourceopen', function() {
    ...
    me.log("using audio codec string for " + me.audio_codec + ": " +
           codec_string);
...
XpraClient.prototype._close_audio_httpstream =
    function() { this._remove_audio_element(); };
...
XpraClient.prototype._close_audio_mediasource = function() {
  this.log("close_audio_mediasource: audio_source_buffer=" +
           this.audio_source_buffer + ", media_source=" + this.media_source +
           ", audio=" + this.audio);
  this.audio_source_ready = false;
  if (this.audio) {
...
XpraClient.prototype._process_sound_data = function(packet, ctx) {
  if (packet[1] != ctx.audio_codec) {
    ctx.error("invalid audio codec '" + packet[1] + "' (expected " +
              ctx.audio_codec + "), stopping audio stream");
    ctx.close_audio();
    return;
  }
...
XpraClient.prototype.add_sound_data = function(codec, buf, metadata) {
  ...
  if (this.audio_buffers.length >= MAX_BUFFERS) {
    this.warn("audio queue overflowing: " + this.audio_buffers.length +
              ", stopping");
  ...
  if (metadata) {
    ...
    for (let i = 0; i < metadata.length; i++) {
      this.debug("audio", "metadata[", i, "]=", metadata[i],
                 ", length=", metadata[i].length,
                 ", type=", Object.prototype.toString.call(metadata[i]));
...
XpraClient.prototype._audio_start_stream = function() {
  this.debug("audio", "audio start of " + this.audio_framework + " " +
                          this.audio_codec + " stream");
  if (this.audio_state == "playing" || this.audio_state == "waiting") {
    // nothing to do: ready to play
    return;
  }
  const me = this;
  this.on_audio_state_change("waiting", "" + this.audio_framework +
                                            " playing " + this.audio_codec +
                                            " stream");
  if (this.audio_framework == "mediasource") {
    ...
    play.then(function(result) { me.debug("audio", "stream playing", result); },
              function(err) {
    ...
    this.on_audio_state_change("error",
...
XpraClient.prototype._audio_ready = function() {
  if (this.audio_framework == "mediasource") {
    ...
    if (this.audio) {
      ...
      this.debug(
          "audio", "audio paused=", this.audio.paused,
          ", queue size=", this.audio_buffers.length,
          ", source ready=", this.audio_source_ready,
...
XpraClient.prototype.push_audio_buffer = function(buf) {
  if (this.audio_framework == "mediasource") {
    ...
    if (b && b.length >= 1) {
      ...
      this.debug("audio", "buffer size=", buf_size,
                 "ms, currentTime=", this.audio.currentTime);
    }
  } else {
 ...
 * Clipboard
 */
XpraClient.prototype.get_clipboard_buffer =
    function() { return this.clipboard_buffer; };
XpraClient.prototype.get_clipboard_datatype =
    function() { return this.clipboard_datatype; };
...
XpraClient.prototype.send_clipboard_token = function(data) {
  ...
  const claim =
      true; // Boolean(navigator.clipboard && navigator.clipboard.readText &&
            // navigator.clipboard.writeText);
  const greedy = true;
  ...
  if (data) {
    packet = [
      "clipboard-token", "CLIPBOARD", [ "UTF8_STRING", "text/plain" ],
      "UTF8_STRING", "UTF8_STRING", 8, "bytes", data, claim, greedy, synchronous
    ];
  } else {
    packet = [
      "clipboard-token", "CLIPBOARD", [], "", "", 8, "bytes", "", claim, greedy,
      synchronous
    ];
  }
  this.send(packet);
...
XpraClient.prototype._process_clipboard_token = function(packet, ctx) {
  ...
  if (packet.length >= 8) {
    ...
    ctx.clipboard_server_buffers[selection] =
  ...
  ctx.debug("clipboard", "dtype=", dtype, "dformat=", dformat,
  ...
  if (is_valid_target) {
    const is_text = dtype.toLowerCase().indexOf("text") >= 0 ||
    ...
    if (is_text) {
        ...
        if (navigator.clipboard && navigator.clipboard.writeText) {
          if (is_text) {
            navigator.clipboard.writeText(wire_data).then(
                ...
                function() { ctx.debug("clipboard", "writeText failed"); });
          }
        }
      }
    } else if (CLIPBOARD_IMAGES && dtype == "image/png" && dformat == 8 &&
               wire_encoding == "bytes" && navigator.clipboard &&
               navigator.clipboard.hasOwnProperty("write")) {
      ...
      navigator.clipboard.write(items)
          ...
          .catch(function(
              err) { ctx.debug("clipboard", "failed to set png image", err); });
    }
  }
};
...
XpraClient.prototype._process_set_clipboard_enabled = function(packet, ctx) {
  ...
  ctx.log("server set clipboard state to " + packet[1] +
          " reason was: " + packet[2]);
};
...
XpraClient.prototype._process_clipboard_request = function(packet, ctx) {
  ...
  const request_id = packet[1], selection = packet[2];
  // target = packet[3];
  ...
  if (navigator.clipboard) {
    if (navigator.clipboard.hasOwnProperty("read")) {
                ...
                if (itemtype == "text/plain") {
                  ...
                  item.getType(itemtype).then(
                      function(blob) {
                        const fileReader = new FileReader();
                        fileReader.onload = function(event) {
                          ctx.send_clipboard_contents(request_id, selection,
                                                      itemtype, 8, "bytes",
                                                      event.target.result);
                        };
                        fileReader.readAsText(blob);
                      ...
                      function(err) {
                        ctx.debug("clipboard",
                                  "getType('" + itemtype + "') failed", err);
                        // send last server buffer instead:
                        ctx.resend_clipboard_server_buffer(request_id,
                                                           selection);
                      });
                  return;
                }
      ...
      navigator.clipboard.readText().then(
          function(text) {
            ctx.debug("clipboard",
                      "clipboard request via readText() text=", text);
            const primary_server_buffer =
                ctx.clipboard_server_buffers["PRIMARY"];
            if (primary_server_buffer && primary_server_buffer[2] == 8 &&
  ...
  ctx.send_clipboard_string(request_id, selection, clipboard_buffer,
                            "UTF8_STRING");
};

XpraClient.prototype.resend_clipboard_server_buffer = function(request_id,
                                                               selection) {
  ...
  this.send_clipboard_contents(request_id, selection, dtype, dformat,
                               wire_encoding, wire_data);
};

XpraClient.prototype.send_clipboard_string = function(
    request_id, selection, clipboard_buffer, datatype) {
  let packet;
  if (clipboard_buffer == "") {
    packet = [ "clipboard-contents-none", request_id, selection ];
  } else {
    packet = [
      "clipboard-contents", request_id, selection, datatype || "UTF8_STRING", 8,
      "bytes", clipboard_buffer
    ];
  }
...
XpraClient.prototype.send_clipboard_contents = function(
    request_id, selection, datatype, dformat, encoding, clipboard_buffer) {
  let packet;
  if (clipboard_buffer == "") {
    packet = [ "clipboard-contents-none", request_id, selection ];
  } else {
    packet = [
      "clipboard-contents", request_id, selection, datatype, dformat || 8,
      encoding || "bytes", clipboard_buffer
    ];
  }
  this.send(packet);
};
...
XpraClient.prototype.save_file = function(filename, data, mimetype) {
  ...
  this.log("saving " + data.length + " bytes of " + mimetype +
...
XpraClient.prototype.print_document = function(filename, data, mimetype) {
  ...
  const win = window.open('data:application/pdf;base64,' + b64data, '_blank');
...
XpraClient.prototype.send_file = function(filename, mimetype, size, buffer) {
  ...
  const packet = [
    "send-file", filename, mimetype, false, this.remote_open_files, size,
    buffer, {}
  ];
  this.send(packet);
};
...
XpraClient.prototype._process_open_url = function(packet, ctx) {
  ...
  if (!new_window || new_window.closed ||
    ...
    const timeout = 10;
    window.doNotification("", 0, summary, body, timeout, null, null, null, null,
                          null);
  }
};
