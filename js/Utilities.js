...
const Utilities = {
  ...
  xorString : function(str1, str2) {
    ...
    for (let i = 0; i < str1.length; i++) {
      result +=
          String.fromCharCode(str1[i].charCodeAt(0) ^ str2[i].charCodeAt(0));
    }
    return result;
  },

  trimString : function(str, trimLength) {
    return str.length > trimLength ? str.substring(0, trimLength - 3) + "..."
                                   : str;
  },
  ...
  getFirstBrowserLanguage : function() {
    const nav = window.navigator, browserLanguagePropertyKeys = [
  ...
  isMacOS : function() { return navigator.platform.includes('Mac'); },

  isWindows : function() { return navigator.platform.includes('Win'); },

  isLinux : function() { return navigator.platform.includes('Linux'); },
  ...
  isEdge : function() { return navigator.userAgent.includes("Edge"); },
  isChrome : function() {
    const isChromium = window.hasOwnProperty("chrome"),
          winNav = window.navigator, vendorName = winNav.vendor,
    ...
    if (uintArray.subarray) {
      for (let i = 0, len = uintArray.length; i < len; i += skip) {
        s += String.fromCharCode.apply(
            null, uintArray.subarray(i, Math.min(i + skip, len)));
      }
    } else {
      for (let i = 0, len = uintArray.length; i < len; i += skip) {
        s += String.fromCharCode.apply(
            null, uintArray.slice(i, Math.min(i + skip, len)));
      }
    }
      ...
      getParameter = function(key) {
        if (!window.location.queryStringParams)
          window.location.queryStringParams =
              Utilities.parseParams(window.location.search.substring(1));
  ...
  getboolparam : function(prop, default_value) {
    ...
    return [ "true", "on", "1", "yes", "enabled" ].indexOf(
  ...
  getConnectionInfo : function() {
    ...
    if (!isNaN(c.downlink) && !isNaN(c.downlink) && c.downlink > 0 &&
        isFinite(c.downlink)) {
      i["downlink"] = Math.round(c.downlink * 1000 * 1000);
    }
    if (c.hasOwnProperty("downlinkMax") && !isNaN(c.downlinkMax) &&
        !isNaN(c.downlinkMax) && c.downlinkMax > 0 && isFinite(c.downlinkMax)) {
