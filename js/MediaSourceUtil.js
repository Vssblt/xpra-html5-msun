...
const MediaSourceUtil = {
  ...
  getMediaSourceClass :
      function() { return window.MediaSource || window.WebKitMediaSource; },
  ...
  getAuroraAudioCodecs : function() {
    ...
    if (AV && AV.Decoder && AV.Decoder.find) {
      for (const codec_option in MediaSourceConstants.AURORA_CODECS) {
        ...
        if (decoder) {
          Utilities.log("audio codec aurora OK  '" + codec_option + "' / '" +
                        codec_string + "'");
          codecs_supported[codec_option] = codec_string;
        } else {
          Utilities.log("audio codec aurora NOK '" + codec_option + "' / '" +
                        codec_string + "'");
        }
      }
    }
    ...
    for (const codec_option in MediaSourceConstants.CODEC_STRING) {
      ...
      try {
        if (!media_source_class.isTypeSupported(codec_string)) {
          Utilities.log("audio codec MediaSource NOK: '" + codec_option +
                        "' / '" + codec_string + "'");
        ...
        if (blacklist.includes(codec_option)) {
          Utilities.log("audio codec MediaSource '" + codec_option + "' / '" +
                        codec_string + "' is blacklisted for " +
                        navigator.userAgent);
          if (ignore_blacklist) {
        ...
        codecs_supported[codec_option] = codec_string;
        Utilities.log("audio codec MediaSource OK  '" + codec_option + "' / '" +
                      codec_string + "'");
      } catch (e) {
        Utilities.error("audio error probing codec '" + codec_string + "' / '" +
                        codec_string + "': " + e);
      }
    }
    Utilities.log("getMediaSourceAudioCodecs(", ignore_blacklist,
                  ")=", codecs_supported);
    return codecs_supported;
  },
  ...
  addMediaSourceEventDebugListeners : function(media_source, source_type) {
    ...
    media_source.addEventListener('sourceopen',
                                  function(e) { debug_source_event('open'); });
    media_source.addEventListener('sourceended',
                                  function(e) { debug_source_event('ended'); });
    media_source.addEventListener('sourceclose',
                                  function(e) { debug_source_event('close'); });
    media_source.addEventListener('error',
                                  function(e) { debug_source_event('error'); });
  },
  ...
  addMediaElementEventDebugListeners : function(media_element, element_type) {
    ...
    media_element.addEventListener('waiting',
                                   function() { debug_me_event("waiting"); });
    media_element.addEventListener('stalled',
                                   function() { debug_me_event("stalled"); });
    media_element.addEventListener('playing',
                                   function() { debug_me_event("playing"); });
    media_element.addEventListener('loadstart',
                                   function() { debug_me_event("loadstart"); });
    media_element.addEventListener(
        'loadedmetadata', function() { debug_me_event("loadedmetadata"); });
    media_element.addEventListener(
        'loadeddata', function() { debug_me_event("loadeddata"); });
    media_element.addEventListener('error',
                                   function() { debug_me_event("error"); });
    media_element.addEventListener('canplay',
                                   function() { debug_me_event("canplay"); });
    media_element.addEventListener('play',
                                   function() { debug_me_event("play"); });
  },
  ...
  addSourceBufferEventDebugListeners : function(asb, element_type) {
    ...
    asb.addEventListener('updatestart',
                         function(e) { debug_buffer_event('updatestart'); });
    asb.addEventListener('updateend',
                         function(e) { debug_buffer_event('updateend'); });
  ...
  get_supported_codecs : function(mediasource, aurora, http,
                                  ignore_audio_blacklist) {
    const codecs_supported = {};
    if (mediasource) {
      const mediasource_codecs =
    ...
    if (http) {
      ...
      for (const i in stream_codecs) {
        ...
        codecs_supported["http-stream:" + codec_option] =
            "http stream: " +
            MediaSourceConstants.CODEC_DESCRIPTION[codec_option];
      }
    }
