...
XpraProtocolWorkerHost.prototype.close =
    function() { this.worker.postMessage({'c' : 'c'}); };

XpraProtocolWorkerHost.prototype.terminate =
    function() { this.worker.postMessage({'c' : 't'}); };

XpraProtocolWorkerHost.prototype.send = function(
    packet) { this.worker.postMessage({'c' : 's', 'p' : packet}); };
...
XpraProtocolWorkerHost.prototype.set_cipher_in = function(
    caps, key) { this.worker.postMessage({'c' : 'z', 'p' : caps, 'k' : key}); };

XpraProtocolWorkerHost.prototype.set_cipher_out = function(
    caps, key) { this.worker.postMessage({'c' : 'x', 'p' : caps, 'k' : key}); };
...
XpraProtocol.prototype.close_event_str =
    function(event) {
  let code_mappings = {
    '1000' : 'Normal Closure',
    '1001' : 'Going Away',
  ...
  this.websocket.onopen =
      function() { me.packet_handler([ 'open' ], me.packet_ctx); };
  ...
  this.websocket.onmessage = function(e) {
    ...
    setTimeout(function() { me.process_receive_queue(); },
...
XpraProtocol.prototype.log =
  ...
  if (proto_flags > 1) {
    this.protocol_error("we can't handle this protocol flag yet: " +
                        proto_flags);
    return;
  }
  ...
  if (this.rQ[0].length == packet_size) {
    ...
    // aggregate all the buffers into "packet_data" until we get exactly
    // "packet_size" bytes:
  ...
  if (level != 0) {
    ...
    if (level & 0x10) {
      ...
      // console.log("lz4 decompress packet size", packet_size, ", lz4 length=",
      // length);
      ...
      if (uncompressedSize <= 0 && packet_size + uncompressedSize != 0) {
        this.protocol_error("failed to decompress lz4 data, error code: " +
                            uncompressedSize);
        return false;
      }
    } else if (level & 0x40) {
  ...
  if (index > 0) {
    ...
    try {
      ...
      if (this.is_worker) {
        ...
        const me = this;
        setTimeout(function() { me.process_message_queue(); },
                   this.process_interval);
      } else {
...
XpraProtocol.prototype.process_send_queue = function() {
  while (this.sQ.length !== 0 && this.websocket) {
    ...
    if (this.cipher_out) {
      proto_flags = 0x2;
      const padding_size = this.cipher_out_block_size -
    ...
    // concat data to header, saves an intermediate array which may or may not
    // have been optimised out by the JS compiler anyway, but it's worth a shot
    header = header.concat(cdata);
    // debug("send("+packet+") "+cdata.length+" bytes in packet for:
    // "+bdata.substring(0, 32)+"..");
...
XpraProtocol.prototype.process_message_queue = function() {
  while (this.mQ.length !== 0) {
    ...
    const raw_draw_buffer = (packet[0] === 'draw') &&
                            (packet[6] !== 'scroll') && (packet[7].buffer);
...
XpraProtocol.prototype.send = function(packet) {
  ...
  const me = this;
  setTimeout(function() { me.process_send_queue(); }, this.process_interval);
};
...
XpraProtocol.prototype.set_cipher_in = function(caps, key) {
  ...
  const secret = forge.pkcs5.pbkdf2(key, caps['cipher.key_salt'],
                                    caps['cipher.key_stretch_iterations'],
                                    this.cipher_in_block_size);
  // start the cipher
...
if (!(typeof window == "object" && typeof document == "object" &&
  ...
  importScripts('lib/bencode.js', 'lib/zlib.js', 'lib/lz4.js',
                'lib/es6-shim.js', 'lib/brotli_decode.js', 'lib/forge.js',
                'lib/pyrencoder.js');
